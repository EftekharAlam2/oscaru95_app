...
class ShopProfileDrink extends StatefulWidget {
  final String id;
  final String tab;

  const ShopProfileDrink({super.key, required this.id, required this.tab});

  @override
  State<ShopProfileDrink> createState() => _ShopProfileDrinkState();
}

class _ShopProfileDrinkState extends State<ShopProfileDrink> {
  @override
  void initState() {
    super.initState();
      ...
      child: StreamBuilder(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            var drinkItem =
                data?.item?.where((item) => item.type == "drink").toList();

            if (drinkItem == null || drinkItem.isEmpty) {
              return Center(
                child: Text(
                  "Food not found!",
                    ...
                    RichText(
                      text: TextSpan(
                        children: [
                          ...
                          TextSpan(
                            text: " (Friday to Monday $openTime - $closeTime)",
