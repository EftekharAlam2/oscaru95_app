...
import 'package:oscaru95/features/user/discover/data/like_rx/api.dart';
...
final class LikePostRx extends RxResponseInt<Map> {
  LikePostRx({required super.empty, required super.dataFetcher});

  ValueStream get getFoodSpecialtream => dataFetcher.stream;
  final api = LikePostAPi.instance;

  Future<bool> like({
    required int id,
  }) async {
    try {
      final data = await api.like(
        id: id,
      );
